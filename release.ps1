...
function Get-GitHubRepository {
  ...
  $normalized = ConvertTo-GitHubRepositorySlug -Value $remote
  if (-not [string]::IsNullOrWhiteSpace($normalized)) {
    return $normalized
  }

  return ConvertTo-GitHubRepositorySlug -Value $ManifestUrl
}
...
$tagName = "v$version"
$repositorySlug = Get-GitHubRepository -RepoRoot $repoRoot -ExplicitRepository $Repository -ManifestUrl $manifest.url
